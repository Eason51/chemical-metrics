...
inputName = "alkProcessed.json"
outputName = "alkFinal.json"

with open(inputName,'r',encoding='utf-8')as fp:
...
for articleDict in drug_molecule_paper:
    vitroDict = articleDict["pharm_metrics_vitro"]
    for key in vitroDict:
        if(vitroDict[key]):
            
            if(key in minVitroDict):
                if(minVitroDict[key] > vitroDict[key]):
                    minVitroDict[key] = vitroDict[key]
            else:
                minVitroDict[key] = vitroDict[key]
            
            if(key in maxVitroDict):
                if(maxVitroDict[key] < vitroDict[key]):
                    maxVitroDict[key] = vitroDict[key]
            else:
                maxVitroDict[key] = vitroDict[key]
    ...
    for key in vivoDict:
        if(vivoDict[key]):
            ...
            if(key in minVivoDict):
                if(minVivoDict[key] > vivoDict[key]):
                    minVivoDict[key] = vivoDict[key]
            else:
                minVivoDict[key] = vivoDict[key]
            
            if(key in maxVivoDict):
                if(maxVivoDict[key] < vivoDict[key]):
                    maxVivoDict[key] = vivoDict[key]
            else:
                maxVivoDict[key] = vivoDict[key]
...
for articleDict in drug_molecule_paper:
    ...
    for key in vitroDict:
        if(vitroDict[key]):
            if(minVitroDict[key] == maxVitroDict[key]):
                normVitro[key] = 0.5
            else:
                normVitro[key] = (vitroDict[key] - minVitroDict[key]) / (maxVitroDict[key] - minVitroDict[key])
    
    normVivo = {}
    vivoDict = articleDict["pharm_metrics_vivo"]
    for key in vivoDict:
        if(vivoDict[key]):
            if(minVivoDict[key] == maxVivoDict[key]):
                normVivo[key] = 0.5
            else:
                normVivo[key] = (vivoDict[key] - minVivoDict[key]) / (maxVivoDict[key] - minVivoDict[key])
    
    articleDict["norm_pharm_metrics_vitro"] = normVitro
    articleDict["norm_pharm_metrics_vivo"] = normVivo


paperCountAdverse1 = set()
paperCountAdverse2 = set()
paperCountAdverse3 = set()

partialCountAdverse1 = set()
partialCountAdverse2 = set()
partialCountAdverse3 = set()
...
for i in drug_molecule_paper:
    ...
    if i['pharm_metrics_vitro']['Ki']!=0.0:
        ...
        metrics_distribution['Ki_Ph'].append(i['pharm_metrics_vitro']['IC50'])
    ...
    if i['clinical_statistics']:
        ...
        index = 0
        for companyList in i['clinical_statistics']['p1_company']:
            for companyName in companyList:
                paperCountAdverse1.add(companyName)
                if(index == 5 or index == 6):
                    partialCountAdverse1.add(companyName)
            index += 1
        ...
        index = 0
        for companyList in i['clinical_statistics']['p2_company']:
            for companyName in companyList:
                paperCountAdverse2.add(companyName)
                if(index == 5 or index == 6):
                    partialCountAdverse2.add(companyName)
            index += 1      
        ...
        index = 0
        for companyList in i['clinical_statistics']['p3_company']:
            for companyName in companyList:
                paperCountAdverse3.add(companyName)    
                if(index == 5 or index == 6):
                    partialCountAdverse3.add(companyName)
            index += 1       
        for j in list(i['clinical_statistics']['p3_adverse_event'].values()):
            metrics_distribution['adverse_3'].append(j)

metrics_paper_count['adverse_1'] = len(paperCountAdverse1)
metrics_paper_count['adverse_2'] = len(paperCountAdverse2)
metrics_paper_count['adverse_3'] = len(paperCountAdverse3)

print(f"p1 5, 6: {len(partialCountAdverse1)}")
print(f"p2 5, 6: {len(partialCountAdverse2)}")
print(f"p3 5, 6: {len(partialCountAdverse3)}")

json_data['metrics_paper_count'] = metrics_paper_count
json_data['metrics_distribution'] = metrics_distribution

with open(outputName,'w', encoding="utf-8")as fp:
