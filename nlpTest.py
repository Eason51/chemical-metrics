...
if(True):
    ...
    ACS.TARGET = "kras"
    acsTableParser = ACS.ACSArticle.TableParser()
    with open(f"files/{ACS.TARGET}/file{acsfileID}.html", encoding="utf-8") as inputFile:
        try:
            acsTableParser.feed(inputFile.read())
        except AssertionError as ae:
            pass
        except Exception as e:
            print(e)

    acsDict = nlp.get_nlp_results(acsTableParser, **modelDict)
    print(acsDict)

    acsTableParser.abstractBoldText


if(False):
    ScienceDirectDOI = ""
    ScienceDirect.TARGET == "kras"

    QUERY_URL = "https://api.elsevier.com/content/article/doi/"
    header = {"X-ELS-APIKey": ScienceDirect.APIKEY, "Accept": "text/xml"}
    response = requests.get(QUERY_URL + ScienceDirectDOI, headers=header)

    sdTableParser = ScienceDirect.ScienceDirectArticle.TableParser()
    try:
        sdTableParser.feed(response.text)
    except AssertionError as ae:
        pass
    except Exception as e:
        print(e)

    sdDict = nlp.get_nlp_results(sdTableParser, **modelDict)
    print(sdDict)
